...
.defines(function() {
	...
	EntityDokuro = ig.Entity.extend({
		...
		init: function(x, y, settings) {
			...
			this.addAnim('climbUp', 0.4, [7,8]);
			this.addAnim('climbDown', 0.4, [7,8]);
		},

		update: function() {
			if (!this.climb) { this.gravityFactor = 1; }
			...
			// jump
			if( (this.standing || this.jump_count < 1 || this.isClimbing || this.canClimb) && (ig.input.pressed('up') ) ) {
