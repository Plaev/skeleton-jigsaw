...
.defines(function() {
	...
	EntityDokuro = ig.Entity.extend({
		...
		update: function() {
			...
			// jump
			if ((this.standing || this.jump_count < 2) && ig.input.pressed('jump')) {
				this.vel.y = -this.jump;
				this.jump_count++;
				if (this.standing) {
					ig.game.spawnEntity(
						EntityDust,
						this.flip ? this.pos.x + this.size.x : this.pos.x-5,
	...
	EntityDokuroHead = ig.Entity.extend({
		...
		animSheet: new ig.AnimationSheet( 'media/dokurohead.png', 6, 6 ),
	...
	EntityDust = ig.Entity.extend({
		...
		animSheet: new ig.AnimationSheet( 'media/dust.png', 6, 6 ),
