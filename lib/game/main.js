...
.defines(function(){
    ...
    SkeletonJigsaw = ig.Game.extend({
        ...
        update: function() {
            ...
            if (ig.game.skeleton.next_level) {
                ...
                if(
                    !(ig.global[SkeletonJigsaw.getCurrentLevel()].entities[ig.global[SkeletonJigsaw.getCurrentLevel()].entities.length -1].type == "EntityVignette"
                    || ig.global[SkeletonJigsaw.getCurrentLevel()].entities[ig.global[SkeletonJigsaw.getCurrentLevel()].entities.length -1].type == "EntityHud")
                ) {
                    ig.global[SkeletonJigsaw.getCurrentLevel()].entities.push({type: "EntityHud", x:0, y:0});
                    ig.global[SkeletonJigsaw.getCurrentLevel()].entities.push({type: "EntityVignette", x:0, y:0});
                }
    ...
    Credits = ig.Class.extend({
        ...
        credits: [
            "Thanks for playing!",
            "Programming by:", "  Willian Molinari (@PotHix)",
            "Design by:", "Rafael Masoni (@rmasoni)"
        ],
        ...
        run: function () {
            ...
            if (delta > 2) {
                ...
                for (var i = 0; i < this.credits.length; i++) {
                    this.font.draw(
                        this.credits[i],
                        ig.system.width / 2 - this.credits[i].length * 4 / 2,
