...
.defines(function(){
    ...
    TitleScreen = ig.Game.extend({
        ...
        init: function() {
            ...
            ig.music.add( 'media/audio/intro.*' );
            ig.music.add( 'media/audio/game.*' );
            ig.music.add( 'media/audio/gameover.*' );
            ig.music.tracks[0].play();
            ig.music.currentTrack.loop = true;
        ...
        update: function() {
            ...
            if (ig.input.pressed(this.startKey) || this.startGameTimer) {
                if (!this.startGameTimer) {
                    this.startGameTimer = new ig.Timer();
                    ig.music.fadeOut(1);
                }

                if (this.startGameTimer.delta() > 1.5) {
                    ig.system.setGame(SkeletonJigsaw);
                }
                return;
            }
        ...
        update: function() {
            ...
            if( dokuro ) {
                ...
                if (ig.game.skeleton.next_level) {
                    ...
                    if (dokuro.animating > ig.system.fps*3) {
    ...
    Credits = ig.Class.extend({
        ...
        init: function () {
            ...
            ig.music.stop();
            ig.music.add( 'media/audio/credits.*' );

            credits_track = ig.music.tracks[ig.music.tracks.length - 1];
            credits_track.loop = true;
            credits_track.play();
