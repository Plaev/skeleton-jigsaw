...
.defines(function(){
    ...
    SkeletonJigsaw = ig.Game.extend({
        ...
        loadLevel: function (level) {
            if (ig.ua.android || ig.ua.iPhone4) {
                // Removing lava animations for mobile
                for (var i = 0; i < level.entities.length; i++) {
                    if (level.entities[i].type === "EntityLavaanimation") {
                        level.entities.erase(level.entities[i]);
                    }
                }

                // Removing doodads layer for mobile
                for (var i = 0; i < level.layer.length; i++) {
                    if (level.layer[i].name === "doodads01") {
    ...
    if (ig.ua.iPad) { // Not just iPad. Should fix for Nexus 7
