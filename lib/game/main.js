...
.defines(function(){
	...
	SkeletonJigsaw = ig.Game.extend({
		...
		update: function() {
			...
			if ( !dokuro ) {
				if ( !this.vignettegameover ) {
					this.vignettefade = ig.game.spawnEntity( EntityVignetteFade, 0, 0 );
				}

				if ( this.vignettefade && this.gameover.delta() > 0) {
					if ( !this.vignettegameover ) {
						this.vignettegameover = ig.game.spawnEntity( EntityVignetteGameover, 0, 0 );
					}
				}
			}
		},
		...
		draw: function() {
			...
			if ( this.vignettefade ) {
				this.vignettefade.pos.x = this.screen.x;
				this.vignettefade.pos.y = this.screen.y;

				if ( this.vignettegameover ) {
					this.vignettegameover.pos.x = this.screen.x;
					this.vignettegameover.pos.y = this.screen.y;
